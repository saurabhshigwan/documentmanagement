package com.jktechproject.documentmanagement.config;
...
public class SecurityConfigClass {
 ...
 private  final UserDetailSErviceImplementation userDetailSErviceImplementation;
    ...
    public SecurityConfigClass(JWTTokenAuthenticator jwtTokenAuthenticator, UserDetailSErviceImplementation userDetailSErviceImplementation) {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable()).authorizeHttpRequests(auth -> auth.
                        requestMatchers(
                                ...
                                "/swagger-ui/**",
                                "/swagger-ui.html"
                        ).permitAll().requestMatchers("/jktech/docman/auth/**").permitAll().requestMatchers("/jktech/docman/doc/**").authenticated())
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authenticationProvider(authenticationProvider())
                .addFilterBefore(jwtTokenAuthenticator, UsernamePasswordAuthenticationFilter.class);
